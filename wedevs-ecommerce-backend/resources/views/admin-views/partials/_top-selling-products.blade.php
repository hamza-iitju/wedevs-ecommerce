        ...
        @foreach ($top_sell as $key => $item)
            @if (isset($item->product))
                <div class="col-md-4 col-6 mt-2"
                    onclick="location.href='{{ url('admin/product/view', [$item['product_id']]) }}'"
                    style="cursor: pointer;padding-right: 6px;padding-left: 6px">
                    <div class="grid-card">
                        <label class="label_1">Sold : {{ $item['count'] }}</label>
                        <center class="mt-4">
                            <img style="height: 90px"
                                src="{{ asset('storage/product/png') }}/{{ $item->product['images'] }}"
                                onerror="this.src='{{ asset('public/assets/back-end/img/160x160/img2.jpg') }}'"
                                alt="{{ $item->product->name }} image">
                        </center>
                        <div class="text-center mt-2">
                            <span class=""
                                style="font-size: 10px">{{ substr($item->product['name'], 0, 20) }}
